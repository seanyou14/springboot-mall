package com.seanyou.springbootmall.dao.impl;
...
public class ProductDaoImpl implements ProductDao{
    ...
    public List<Product> getProducts(ProductCategory category, String search) {
        ...
        if (category != null) {
            sql = sql + " AND category = :category";  // AND前預留空白
            map.put("category", category.name());
        }
                                                    // DAO層實作查詢條件時值不是null時
        if (search != null) {                       // 去給出對應的語句拼接查詢
            sql = sql +" AND product_name LIKE :search";
            map.put("search", "%"+ search + "%");   // 模糊查詢%不能寫在SQL語句裡面,需放在map裡面
        }
